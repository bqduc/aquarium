...
public class DropboxServiceManager {
   ...
   * @param inputStream local file to upload
   * @param dropboxPath Where to upload the file to within Dropbox
   */
  private FileMetadata uploadFile(DbxClientV2 dbxClient, InputStream inputStream, String dropboxFolder, String dropboxPath) throws DropboxException {
  	FileMetadata metadata = null;
  	try {
        metadata = dbxClient
        		.files()
        		.uploadBuilder(dropboxFolder + "/" + dropboxPath)
            .withMode(WriteMode.ADD)
            .withClientModified(GregorianCalendar.getInstance().getTime())
   ...
   * @param dropboxPath Where to upload the file to within Dropbox
   */
  public void uploadFile(DropboxUser dropboxUser, InputStream inputStream, String dropboxFolder, String dropboxPath) throws DropboxException {
  	DbxClientV2 dbxClient = dropboxClientServiceFactory.getDropboxClient();
  	FileMetadata metadata = this.uploadFile(dbxClient, inputStream, dropboxFolder, dropboxPath);
